...
for name in os.listdir(ENCODINGS_DIR):
    ...
    for filename in os.listdir("{}/{}".format(ENCODINGS_DIR,name)):
        ...
        encoding = pickle.load(open('{}/{}/{}'.format(ENCODINGS_DIR, name, filename), 'rb'))
...
New Distance: '''))
    ...
    for face_encoding, face_location in zip(encodings, locations):
        ...
        if True in results:  # If at least one is true, get a name of first of found labels
            ...
            pkl_file = open('{}/{}/{}-{}.pkl'.format(ENCODINGS_DIR, match, match, int(time.time())), 'wb')
...
#if theres something copied, or else no point running this
shutil.disk_usage(ENCODINGS_DIR)#Just so that shutil moves out of the last directory that i want to delete
